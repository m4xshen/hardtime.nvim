...
local default_config = {
   ...
   fmt_msg = function(is_console, mode_name, msg)
   ...
   local log_at_level = function(level, level_config, message_maker, ...)
      ...
      if config.use_console then
         local log_to_console = function()
            local console_string = config.fmt_msg(true, level_config.name, msg)
            ...
            for _, v in ipairs(split_console) do
               ...
               local ok =
                  pcall(vim.cmd, string.format([[echom "%s"]], formatted_msg))
               if not ok then
